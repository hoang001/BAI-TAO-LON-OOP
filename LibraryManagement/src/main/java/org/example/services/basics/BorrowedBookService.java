...
/**
 * Lớp BorrowedbookDao chịu trách nhiệm quản lý các thao tác liên quan đến sách đã mượn, bao gồm
 * mượn sách, trả sách, tìm sách chưa trả, lấy danh sách sách đã mượn theo khoảng thời gian, và các
 * thao tác khác liên quan đến sách mượn của người dùng.
 */
public class BorrowedBookService {
  ...
  /**
   * Constructor của lớp BorrowedBookService.
  ...
  public boolean borrowBook(int bookId, LocalDate borrowDate, LocalDate returnDate) {
    try {
      ...
      Future<List<BorrowedBookEntity>> futureOverdueBooks =
          executorService.submit(
              () ->
                  borrowedBookDao.findOverdueBooksByUser(
                      userService.getLoginUser().getUserName(), LocalDate.now()));
      List<BorrowedBookEntity> overdueBooks =
          futureOverdueBooks.get(5, TimeUnit.SECONDS);

      if (!overdueBooks.isEmpty()) {
        throw new IllegalStateException(
            "Bạn có sách quá hạn. Vui lòng trả sách trước khi mượn sách mới.");
      }
      ...
      // Ghi nhận mượn sách
      Future<Boolean> futureBorrow =
          executorService.submit(
              () ->
                  borrowedBookDao.borrowBook(
                      bookId, userService.getLoginUser().getUserName(), borrowDate, returnDate));
      boolean result = futureBorrow.get(5, TimeUnit.SECONDS);

      if (result) {
        // Cập nhật trạng thái sách nếu chỉ còn 1 quyển
        if (book.getQuantity() - borrowedCount == 1) {
          Future<Boolean> futureUpdate =
              executorService.submit(() -> bookDao.updateBookAvailability(bookId, false));
        ...
        logDao.addLog(
            new LogEntity(
                ...
                "Mượn sách ID: " + bookId + " thành công"));
      } else {
  ...
  public boolean returnBook(int bookId) {
    try {
      ...
      // Ghi nhận trả sách
      Future<Boolean> futureReturn =
          executorService.submit(
              () -> borrowedBookDao.returnBook(bookId, userService.getLoginUser().getUserName()));
      boolean result = futureReturn.get(5, TimeUnit.SECONDS);

      if (result) {
        // Cập nhật trạng thái sách luôn là available
        Future<Boolean> futureUpdate =
            executorService.submit(() -> bookDao.updateBookAvailability(bookId, true));
        if (!futureUpdate.get(5, TimeUnit.SECONDS)) {
          throw new IllegalStateException("Không thể cập nhật trạng thái sách.");
        }
        logDao.addLog(
  ...
  public List<BorrowedBookEntity> getNotReturnedBooksByUser() {
    try {
      ...
      Future<List<BorrowedBookEntity>> futureBooks =
          executorService.submit(
              () -> {
                try {
                  List<BorrowedBookEntity> books =
                      borrowedBookDao.findNotReturnedBooksByUser(
                          userService.getLoginUser().getUserName());
                  logDao.addLog(
                      new LogEntity(
                          LocalDateTime.now(),
                          userService.getLoginUser().getUserName(),
                          "Tìm thấy sách chưa trả thành công"));
                  return books;
                  ...
                  System.out.println(
                      "Lỗi cơ sở dữ liệu trong quá trình tìm sách chưa trả: " + e.getMessage());
                  return null;
                }
              });
      return futureBooks.get(5, TimeUnit.SECONDS);
   ...
   * @return Danh sách các đối tượng BorrowedBookEntity hoặc null nếu có lỗi.
   */
  public List<BorrowedBookEntity> getBorrowedBooksByUserAndDateRange(
      ...
      Future<List<BorrowedBookEntity>> futureBooks =
          executorService.submit(
              () -> {
                try {
                  List<BorrowedBookEntity> books =
                      borrowedBookDao.findBorrowedBooksByUser(
                          userService.getLoginUser().getUserName(), startDate, endDate);
                  logDao.addLog(
                      new LogEntity(
                          LocalDateTime.now(),
                          userService.getLoginUser().getUserName(),
                          "Tìm thấy sách mượn trong khoảng thời gian thành công"));
                  return books;
                } catch (SQLException e) {
                  logDao.addLog(
                      new LogEntity(
                          ...
                          "Lỗi cơ sở dữ liệu: " + e.getMessage()));
                  System.out.println(
                      "Lỗi cơ sở dữ liệu trong quá trình tìm sách mượn: " + e.getMessage());
                  return null;
                }
              });
      return futureBooks.get(5, TimeUnit.SECONDS);
  ...
  public int getBorrowedBooksCountByUser() {
    try {
      ...
      Future<Integer> futureCount =
          executorService.submit(
              () -> {
                try {
                  int count =
                      borrowedBookDao.findBorrowedBooksCountByUser(
                          userService.getLoginUser().getUserName());
                  logDao.addLog(
                      new LogEntity(
                          LocalDateTime.now(),
                          userService.getLoginUser().getUserName(),
                          "Lấy số lượng sách đã mượn thành công"));
                  return count;
                } catch (SQLException e) {
                  logDao.addLog(
                      new LogEntity(
                          ...
                          "Lỗi cơ sở dữ liệu: " + e.getMessage()));
                  System.out.println(
                      "Lỗi cơ sở dữ liệu trong quá trình lấy số lượng sách đã mượn: "
                          + e.getMessage());
                  return 0;
                }
              });
      return futureCount.get(5, TimeUnit.SECONDS);
    } catch (InterruptedException | ExecutionException | TimeoutException e) {
      System.out.println("Lỗi: " + e.getMessage());
      try {
        logDao.addLog(
      ...
      Future<List<BorrowedBookEntity>> futureBooks =
          executorService.submit(
              () -> {
                try {
                  List<BorrowedBookEntity> books =
                      borrowedBookDao.findAllBorrowedBooksByUser(
                          userService.getLoginUser().getUserName());
                  logDao.addLog(
                      new LogEntity(
                          LocalDateTime.now(),
                          userService.getLoginUser().getUserName(),
                          "Lấy danh sách sách đã mượn thành công"));
                  return books;
                } catch (SQLException e) {
                  logDao.addLog(
                      new LogEntity(
                          ...
                          "Lỗi cơ sở dữ liệu: " + e.getMessage()));
                  System.out.println(
                      "Lỗi cơ sở dữ liệu trong quá trình lấy danh sách sách đã mượn: "
                          + e.getMessage());
                  return null;
                }
              });
