...
/**
 * Lớp LogService chịu trách nhiệm quản lý các thao tác liên quan đến nhật ký (log). Bao gồm thêm
 * log, lấy log theo ID, theo người dùng, hoặc theo khoảng thời gian.
 */
public class LogService {
  ...
  /**
   * Constructor của lớp LogService.
  ...
  public List<LogEntity> getAllLogs() {
    try {
      ...
      Future<List<LogEntity>> futureLogs =
          executorService.submit(
              () -> {
                try {
                  ...
                  System.out.println(
                      "Lỗi cơ sở dữ liệu trong quá trình lấy tất cả log: " + e.getMessage());
                  return null;
                }
              });
      return futureLogs.get(5, TimeUnit.SECONDS);
          ...
          executorService.submit(
              () -> {
                try {
                  ...
                  // Xử lý lỗi cơ sở dữ liệu và ghi log
                  System.out.println(
                      "Lỗi cơ sở dữ liệu trong quá trình lấy log theo ID: " + e.getMessage());
                  return null;
                }
              });
      return futureLog.get(5, TimeUnit.SECONDS);
    } catch (InterruptedException | ExecutionException e) {
          ...
          executorService.submit(
              () -> {
                try {
                  ...
                  // Xử lý lỗi cơ sở dữ liệu và ghi log
                  System.out.println(
                      "Lỗi cơ sở dữ liệu trong quá trình lấy log theo userName: " + e.getMessage());
                  return null;
                }
              });
      return futureLogs.get(5, TimeUnit.SECONDS);
          ...
          executorService.submit(
              () -> {
                try {
                  ...
                  // Xử lý lỗi cơ sở dữ liệu và ghi log
                  System.out.println(
                      "Lỗi cơ sở dữ liệu trong quá trình lấy log theo thời gian: "
                          + e.getMessage());
                  return null;
                }
              });
